...
# ─── Combined explainable (Task 4a) — always-on schema, data-gated explain ──
def test_explanation_schema_combined_returns_200():
    r = client.get("/strategies/combined_explainable/explanation_schema")
    assert r.status_code == 200
    body = r.json()
    assert body["strategy"] == "combined_explainable"
...
def test_explanation_schema_other_strategy_404():
    r = client.get("/strategies/ma_crossover/explanation_schema")
    assert r.status_code == 404
    assert "combined_explainable" in r.json()["detail"]


@ch_required
def test_backtest_explain_returns_explanations():
    """End-to-end /backtest/explain — uses the same default-child line-up
    the strategy ships with. Skipped without ClickHouse per the project's
    'no mocking the database' principle."""
    ticker = CH_TICKERS[0]
    r = client.post(
        "/backtest/explain",
        json={
            ...
            "end": "2023-01-01",
            "interval": "1d",
            # `strategy` is ignored by /backtest/explain but the schema
            # requires the field.
            "strategy": "combined_explainable",
            "params": {"min_active_children": 1},
            "commission": 0.0005,
            "slippage": 0.0005,
        },
    )
    ...
    # Either we got trades (and explanations) or none — both shapes valid.
    assert isinstance(body["explanations"], list)
    for e in body["explanations"]:
        assert {"ticker", "timestamp", "direction", "weights", "child_signals", "summary"}.issubset(e)
